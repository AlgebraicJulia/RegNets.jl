# # Demo for Signed Graphs as a Syntax for generalized Lotka Volterra

# This demo shows how to use a combinatorial object (signed graphs) as a syntax for 
# a mathematical object (ODEs for Lotka Volterra dynamics). The combinatorial structure
# is the data structure that backs a graphical language. We will implement that graphical
# language with Graphviz that can render the models. There is a GUI interface in SemanticModels.jl
# that can allow for user input of arbitrary combinatorial structures. 

# This demo also uses StructuredMulticospans as a formalism for modeling the kind of combination that
# you do when you glue models together by sharing variables.
...
# We need to tell Catlab that positive edges are green and negative edges are red.
...
If all the pfunc and nfunc functions are constant, this
matrix would give you linear system dynamics.
...
The matrix A is allowed to depend on u, pass a SignedGraph where all the pfunc and nfunc functions are constant to recover
the standard definition of GLV dynamics. 

   u̇ = u .* (r+Au)

In our semantics A[j,i] can depend on u[i]. You could have A[j,i] depend on u[i], and u[j].
Anything more flexible than this would prevent relating the structure of the graph to the behavior of the system.
At that point, you should switch to a hypergraph representation like Petri nets for mass action kinetics.
"""
...
# # Step 1: Specify the signed graph. 
 
# The functional form of the rates get captured as functions that you store in the pfunc and nfunc columns of the database.
...
# You can tell Catlab how to draw this using our previous drawing scheme and adding in the input and outputs as vertices with blue arrows.

R = OpenSG{Function}(sglv, FinFunction([1],3), FinFunction([3],3))
propertygraph(R::StructuredCospan) = begin
    interface(R) = reduce(copair, [l[:V] for l in  legs(R)])
    pg = propertygraph(apex(R))
    iface = interface(R)
    for x in dom(iface)
        v = add_vertex!(pg, Dict(:shape=>"none", :label=>string(x)))
        @show x, v, iface(x)
        add_edge!(pg, v, iface(x), Dict(:color => "blue", :style=>"dashed"))
        end
    return pg
end

propertygraph(R::StructuredMulticospan) = begin
    interface(R) = reduce(copair, [l[:V] for l in  legs(R)])
    pg = propertygraph(apex(R))
    iface = interface(R)
    for x in dom(iface)
...
to_graphviz(term, box_labels=:name, junction_labels=:variable) |> show

#-

m = oapply(term, Dict(:ffn=>ffn, :feedback=>fb))
to_graphviz(propertygraph(m))

# TODO: Fix the rest of this example notebook
#
# # Notice that the result of oapply has a boundary of two vertices.
# # The `@relation` macro allows you to specify the external boundary of the composite,
# # which allows you to use a composite system as a primitive system in another layer of composition.
# # This is the key property of being an operad algebra. We can compose our wiring diagrams by replacing bubbles with systems
# # and then apply the operad term to a collection of primitive systems, 
# # or we can compose up our systems and then compose the composites and
# # we always get the same system regardless of the execution order of the compositions.
# # 
# # ## Exercise
# # 
# # Choose initial conditions and rates to simulate the composite model m.
#
